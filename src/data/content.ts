export const PROFILE = {
  nameEn: 'Kyungdeok',
  nameKo: '박경덕',
  role: 'AX Creator',
  company: '(주)풀무원 AX혁신실',
  email: 'sweetgump@naver.com',
  linkedin: 'https://www.linkedin.com/in/%EA%B2%BD%EB%8D%95-%EB%B0%95-2ab536196/',
  tagline: 'AI 에이전트 · 업무 자동화 · 데이터 시각화로 일하는 방식을 다시 설계합니다',
  about:
    '좋은 AI 에이전트를 만드는 것보다, 그걸 현업에서 안전하게 굴리는 게 더 어렵습니다. 기업의 AI 전환(AX) 현장에서 저는 매일 이 문제를 풉니다. 풀무원 AX혁신실에서 AI · 데이터 · 자동화 과제를 직접 기획하고 구현하며, 현업이 스스로 도구를 만드는 시민개발자 문화를 조직 전반으로 넓히고 있습니다. 그리고 현장에서 검증한 것만 강단으로 잇습니다.',
};

export const NAV_LINKS = [
  { label: 'About', href: '#about' },
  { label: 'Capabilities', href: '#capabilities' },
  { label: 'Courses', href: '#courses' },
  { label: 'Contact', href: '#contact' },
];

export const SERVICES = [
  {
    number: '01',
    name: 'Data Visualization',
    description:
      'Power BI · Tableau로 흩어진 데이터를 하나의 흐름으로 모으고, 의사결정에 바로 쓰이는 대시보드를 설계합니다.',
  },
  {
    number: '02',
    name: 'Process Automation',
    description:
      'Power Automate 기반 RPA로 반복 업무를 자동화하고, 자동화를 핑계 삼아 프로세스 자체를 다시 설계합니다.',
  },
  {
    number: '03',
    name: 'AI Agent',
    description:
      'Copilot Studio · n8n 기반 AI 에이전트를 PoC에서 멈추지 않고 실제 운영 가능한 사내 서비스로 만듭니다.',
  },
  {
    number: '04',
    name: 'AX Consulting',
    description:
      '전사 AX/DX 과제를 기획부터 실행까지 책임집니다. 기술보다 먼저 문제 정의와 현업 수용성을 봅니다.',
  },
  {
    number: '05',
    name: 'Education',
    description:
      '현업이 스스로 도구를 만드는 시민개발자 문화를 확산합니다. 사내 강의와 Udemy 정규 강좌를 기획·운영합니다.',
  },
];

export const PROJECTS = [
  {
    number: '01',
    category: 'Udemy Course',
    name: 'Power Automate Desktop',
    summary: '코딩 없이 반복 업무를 자동화하는 RPA 실무 입문 과정',
    href: 'https://www.udemy.com/course/power-automate-desktop-class/',
    image: '/courses/power-automate-desktop.jpg',
    tiles: ['RPA 자동화', '실무 예제 중심'],
    accent: ['#0B1220', '#2A5CAA', '#4FA3E3'],
  },
  {
    number: '02',
    category: 'Udemy Course',
    name: 'Power BI Desktop',
    summary: '데이터 연결부터 모델링, DAX, 대시보드 배포까지 한 번에',
    href: 'https://www.udemy.com/course/power-bi-desktop/',
    image: '/courses/power-bi-desktop.jpg',
    tiles: ['데이터 모델링', 'DAX · 대시보드'],
    accent: ['#161006', '#7A5E00', '#C79A00'],
  },
  {
    number: '03',
    category: 'Udemy Course',
    name: '경영정보시각화능력 실기',
    summary: '국가공인 자격 실기 대비, 기출 유형별 문제 풀이',
    href: 'https://www.udemy.com/course/power-bi-exam/',
    image: '/courses/power-bi-exam.jpg',
    tiles: ['자격 실기 대비', '기출 유형 분석'],
    accent: ['#111111', '#4A4133', '#8C7A55'],
  },
];
